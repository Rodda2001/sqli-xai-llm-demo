...
explainer1 = None
try:
    if SHAP_BG_PATH and __import__("os").path.exists(SHAP_BG_PATH):
...
def get_llm_explanation(report: dict) -> str:
    ...
    prompt = f"""You are a senior SOC analyst writing a concise but descriptive incident explanation for a detected SQL injection attack.

Analyze the following detection result and produce a clear explanation for a technical user.
...
Write EXACTLY in this format with these 5 sections:

Threat Summary:
[2 short sentences. Explain what the attacker is trying to do and why this query is dangerous.]

Why It Was Flagged:
[2 short sentences. Mention the exact SQLi behavior and reference the suspicious patterns found.]

Potential Impact:
[2 short sentences. Explain what could happen if this attack succeeds, such as authentication bypass, data theft, database enumeration, modification, or command execution.]

Key Signals:
[One line, comma-separated list of the most important suspicious tokens/patterns.]

Recommended Action:
[2 short sentences. Give the exact immediate defensive action a SOC analyst or developer should take.]

Rules:
- Be specific to THIS query.
- Do not be generic.
- Do not mention uncertainty unless necessary.
- Keep total output under 170 words.
- Sound like a real security incident explanation.
"""
    ...
    try:
        ...
        if not OPENROUTER_API_KEY:
            ...
            return "[FALLBACK]\n" + generate_fallback_explanation(report)
        ...
        if response.status_code == 200:
            ...
            if text and len(text) > 20:
                ...
                disclaimer = (
                    f"\n\n---\n"
                    f"This explanation was generated by AI model ({OPENROUTER_MODEL}) at {generated_at}. "
                    f"It may contain inaccuracies and should be reviewed before making security decisions."
                )
                final_text = "[LIVE LLM]\n" + text + disclaimer

                log.info(f"LLM explanation generated via OpenRouter ({len(final_text)} chars)")
                return final_text
...
def generate_fallback_explanation(report: dict) -> str:
    ...
    mitre_text = "unknown MITRE mapping"
    if mitre:
        mitre_text = f"MITRE {mitre['technique']} — {mitre['name']}"

    return (
        f"Threat: {desc[0]} detected with {conf:.1f}% confidence ({mitre_text}).\n"
        f"Reason: {desc[1]}.\n"
